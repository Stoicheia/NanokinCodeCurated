...
[System.Serializable]
public class LayoutMaxSize : UnityEngine.UI.LayoutElement
{
    ...
    public void UpdateMaxSizes()
    {
        if (maxHeight != -1f)
        {
            ...
            else if (preferredHeight != -1f && transform.childCount > 0)
            {
                bool first = true;
                float biggestY = 0;
                float lowestY = 0;
	    ...
	    //Debug.Log($"Update Max Sizes {preferredWidth},{preferredHeight}");
    }
}
...
public class LayoutMaxSizeEditor : Editor
{
    public override void OnInspectorGUI()
    {
        ...
        if (!layoutMax.ignoreLayout)
        {
	        ...
	        GUILayout.Label("Preferred Width:");
	        GUILayout.FlexibleSpace();
	        GUILayout.Label(layoutMax.preferredWidth.ToString());
	        GUILayout.EndHorizontal();


	        GUILayout.BeginHorizontal();
	        GUILayout.Label("Preferred Height:");
	        GUILayout.FlexibleSpace();
	        GUILayout.Label(layoutMax.preferredHeight.ToString());
	        GUILayout.EndHorizontal();

	        if(GUILayout.Button("CalculateLayoutInputHorizontal")) layoutMax.CalculateLayoutInputHorizontal();
	        if(GUILayout.Button("CalculateLayoutInputVertical")) layoutMax.CalculateLayoutInputVertical();
	        if(GUILayout.Button("UpdateMaxSizes")) layoutMax.UpdateMaxSizes();
        }
    }
}
#endif
